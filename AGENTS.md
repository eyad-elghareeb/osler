...
*Scroll-aware chrome*: `useHideOnScroll(retryKey)` from `@/hooks/use-hide-on-scroll.ts` is the only sanctioned hide-on-scroll implementation (mobile scroll-away bar, QBank header). It listens in capture phase for `.osler-page` scrollers (works with late-mounted/growing content — no polling). Its core defense is **layout-shift filtering**: any scroll event whose scrollable range changed is browser clamp/reflow noise (collapsing the chrome resizes the scroller and the browser phantom-adjusts scrollTop) and is ignored — without this, just-tall-enough pages oscillate forever. On top of that: collapse requires ≥140px range at collapse time, always expanded near the top, asymmetric 24/48px bands, direction-reset accumulators, 280ms toggle cooldown, ResizeObserver expand if the page becomes unscrollable. Bars animate height+opacity via framer (`initial={false}`, `overflow-hidden`) — never hand-roll delta-threshold listeners or animate with CSS `transition-all`.
