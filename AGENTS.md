...
*Scroll-aware chrome*: `useHideOnScroll(retryKey, { reservePx })` from `@/hooks/use-hide-on-scroll.ts` is the only sanctioned hide-on-scroll implementation (mobile scroll-away bar, QBank header). It listens in capture phase for `.osler-page` scrollers (works with late-mounted/growing content — no polling) and keeps hysteresis state **per element** (WeakMap), so NavigationStack views with two stacked `.osler-page` layers (home + subpage) never pollute each other's gesture state. Its core defense is **clamp-signature filtering**: a scroll event is ignored only when the range shrank AND scrollTop is pinned to the new max (the browser-forced clamp after chrome collapse); other range changes (late images, lazy grids, filtered lists) flow through normally. Collapse additionally requires **post-collapse headroom**: callers pass `reservePx` = total layout px ALL collapsible chrome on that scroller reclaims (app bar 54px; QBank hub header passes 150 covering both), and collapse fires only if the predicted post-collapse range clears 80px — otherwise the clamp parks scrollTop in the always-expanded zone and the bars spring back. On top of that: ≥140px range at collapse time, always expanded ≤40px from top, asymmetric 24/48px bands, direction-reset accumulators, 280ms toggle cooldown, and a ResizeObserver over every seen scroller that expands only when the ACTIVE (last-scrolled) one becomes unscrollable. Bars animate height+opacity via framer (`initial={false}`, `overflow-hidden`, `MOTION_TRANSITION.fast`) — never hand-roll delta-threshold listeners or animate with CSS `transition-all`.
