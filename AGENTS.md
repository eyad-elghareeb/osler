...
*Scroll-aware chrome*: `useHideOnScroll(retryKey)` from `@/hooks/use-hide-on-scroll.ts` is the only sanctioned hide-on-scroll implementation (mobile scroll-away bar, QBank header). It listens in capture phase for `.osler-page` scrollers (works with late-mounted/growing content — no polling). Collapse requires ≥140px of scroll range *at collapse time*; the rule is one-sided on purpose — a post-collapse range dip never auto-reveals (that feedback loop caused oscillation on short pages), only upward intent, reaching the top, or the page becoming unscrollable (ResizeObserver) does. Bars snap (conditional render) + 0.15s compositor-only entrance — never animate their height, and never re-implement delta-threshold scroll listeners.
