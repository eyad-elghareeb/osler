...
- **Gmail SMTP relay worker** (`cloudflare/email-worker`) — a standalone Worker that sends transactional email through Gmail's SMTP servers from a plain Gmail account + App Password: no custom domain, no DNS records, no email service to pay for. Hardened beyond a textbook SMTP client (multiline/chunked reply reads, TLS socket re-bind after STARTTLS, RFC 2045/2047 encoding for Arabic subjects, dot-stuffing) and bearer-protected by a shared `EMAIL_TOKEN` so it cannot be used as an open relay. The main Worker's `sendEmail` now dispatches: `EMAIL_WORKER_URL` + `EMAIL_WORKER_TOKEN` (Gmail relay, takes precedence) → `RESEND_API_KEY` + `EMAIL_FROM` (Resend). Setup guide: `cloudflare/email-worker/README.md`.
...
- **Tauri admin is now purely an instance generator & manager** — the Content Studio (CMS) was removed (views, entrypoint, EasyMDE/Mermaid editors, manifest/validate Rust modules). The 5-step Instance Generator is now a full A-to-Z assisted setup: the Cloudflare pipeline provisions D1/R2/Worker/Pages with `JWT_SECRET` and patches `cloud.apiUrl`, optional **Google Sign-In** credentials are collected in the wizard and written to the Worker as secrets (the exact Authorized redirect URI is shown for the Google Cloud Console), the Ready step adds a backend **health check** and **first-admin promotion** (new `setup_*` Tauri commands). The desktop app no longer edits content.
