...
if('serviceWorker' in navigator){{window.addEventListener('load',function(){{navigator.serviceWorker.register('{sw_path}').catch(function(){{}});}});}}
        ...
        back_btn = back_btn,
        footer = engine_assets::FOOTER_NOTE,
        q_json = q_json,
...
pub fn build_project_zip(config: &ProjectConfig) -> Result<Vec<u8>, String> {
    ...
    // Engine files
    add_str(&mut zip, "index-engine.js", engine_assets::INDEX_ENGINE_JS)?;
    add_str(&mut zip, "index-engine.css", engine_assets::INDEX_ENGINE_CSS)?;
    add_str(&mut zip, "search-engine.js", engine_assets::SEARCH_ENGINE_JS)?;
    add_str(&mut zip, "engine-shared.js", engine_assets::ENGINE_SHARED_JS)?;
    add_str(&mut zip, "engine-shared.css", engine_assets::ENGINE_SHARED_CSS)?;
    add_str(&mut zip, "engine-tracker.js", engine_assets::ENGINE_TRACKER_JS)?;
    add_str(&mut zip, "quiz-engine.js", engine_assets::QUIZ_ENGINE_JS)?;
    add_str(&mut zip, "bank-engine.js", engine_assets::BANK_ENGINE_JS)?;
    // Phase 6.5 fix #1: sync-engine.js no longer bundled.
    add_str(&mut zip, "flashcard-engine.js", engine_assets::FLASHCARD_ENGINE_JS)?;
    add_str(&mut zip, "written-engine.js", engine_assets::WRITTEN_ENGINE_JS)?;
    add_str(&mut zip, "osce-engine.js", engine_assets::OSCE_ENGINE_JS)?;
    add_str(&mut zip, "ai-assistant-engine.js", engine_assets::AI_ASSISTANT_ENGINE_JS)?;

    // Static assets
    add_str(&mut zip, "assets/favicon.svg", engine_assets::FAVICON_SVG)?;
    add_str(&mut zip, "netlify.toml", engine_assets::NETLIFY_TOML)?;
    add_str(&mut zip, "vercel.json", engine_assets::VERCEL_JSON)?;
    add_str(&mut zip, "manifest.webmanifest", &make_manifest(project_name))?;

    // .gitignore
    add_str(&mut zip, ".gitignore", engine_assets::GITIGNORE_CONTENT)?;

    // GitHub workflows
    let _ = zip.add_directory(".github/", opts);
    let _ = zip.add_directory(".github/workflows/", opts);
    add_str(&mut zip, ".github/workflows/sync-quiz-assets.yml", engine_assets::SYNC_WORKFLOW_YML)?;
    add_str(&mut zip, ".github/workflows/jekyll-gh-pages.yml", engine_assets::DEPLOY_WORKFLOW_YML)?;

    // Scripts directory
    let _ = zip.add_directory("scripts/", opts);
    add_str(&mut zip, "scripts/sync_quiz_assets.py", engine_assets::SYNC_SCRIPT)?;
    add_str(&mut zip, "scripts/standardize_quiz_files.py", engine_assets::STANDARDIZE_SCRIPT)?;
    ...
    add_bytes(&mut zip, admin_filename, engine_assets::OSLER_ADMIN_BINARY)?;
