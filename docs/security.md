...
- **Email verification gate**: signup is username-only (the address is added later in Settings, which mails a verification link at once). Password sign-in is blocked until the email address is verified (a fresh link goes out with the rejection and on registration) — mass-registered bot accounts stay inert with no session. Username-only accounts have nothing to verify and are unaffected; Google-provisioned accounts are stamped at link time. Unverified addresses also still block Google sign-in linking (anti-account-jacking guard).
