...
- **Email verification gate**: signup takes an optional email address (for password recovery) — accounts can be created without one, but an account that sets one must verify it: emailed signups get no session until they click the link, and password sign-in stays blocked while unverified (a fresh link goes out with the rejection). Mass-registered bot accounts stay inert with no session. Username-only accounts have nothing to verify and are unaffected; Google-provisioned accounts are stamped at link time. Unverified addresses also still block Google sign-in linking (anti-account-jacking guard).
