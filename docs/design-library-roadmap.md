...
- **App-wide English typography baseline** (Osler) — previously the editorial refinements (`text-wrap: balance`, hanging punctuation, tabular figures, etc.) only applied to `.uworld-prose`/`.library-article`/`.medos-article`. Added to `@layer base` in `globals.css`, so it now applies everywhere by inheritance/attribute-selector rather than only in long-form prose:
