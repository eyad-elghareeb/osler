...
| --- | --- | --- | --- | --- |
| P0 | Motion audit and shared recipes | Replace the remaining ad-hoc CSS entrance animations with purposeful, reduced-motion-safe Framer Motion patterns. | Motion Primitives | **Done** — single source of truth established: `MOTION_DURATION`/`MOTION_EASE`/`MOTION_SPRING`/`MOTION_TRANSITION` tokens + `@/components/osler/motion-primitives.tsx` (`FadeIn`, `Stagger`/`StaggerItem`, `Pressable`, `HoverLift`). All inline `duration`/`ease`/`spring` values across ~40 files unified to tokens. Site-wide infinite pulse on `PageHeader` removed per frequency gate. `osler-fade-in` CSS keyframe unified to the standard curve. Ambient loops (skeleton shimmer, voice orb) preserved as functional state indicators. |
