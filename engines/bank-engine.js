...
function selectAnswer(qIdx, optIdx) {
  ...
  // V20 analytics — bank-engine was missing this entirely (H7 fix).
  if (window.OslerAnalytics) {
    var correctIdx = SESSION_QUESTIONS[qIdx] && SESSION_QUESTIONS[qIdx].correct;
    OslerAnalytics.trackAnswer('bank', BANK_CONFIG.uid, qIdx, optIdx === correctIdx ? 'correct' : 'wrong');
  }
  EngineShared.debounceSave(saveProgress);
  updateNavGrid(qIdx);
  updateNavStats();
...
function buildDashQItem(uid, q, typeLabel, iconClass, iconText) {
  ...
  return '<div class="dash-q-item">'
    ...
    + '</div>'
    + '<button type="button" class="dash-q-remove" onclick="removeTrackerItem(\'' + uid + '\',' + (q.idx || 0) + ')" title="Remove" aria-label="Remove">'+EngineShared.icon('x')+'</button>'
    + '</div>';
}
